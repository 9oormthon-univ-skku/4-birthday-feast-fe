...
export default function QuizPage() {
  ...
  const [questions, setQuestions] = useState<QuizQuestion[]>(sorted);
  const [answers, setAnswers] = useState<boolean[]>(sorted.map((q) => q.answer));
  ...
  const handleToggleEditMode = () => {
    if (editMode) {
      const payload: QuizData = {
        quizId: initialQuizData.quizId,
        birthdayId: initialQuizData.birthdayId,
        questions,
      };
      console.log('[Quiz Save Payload]', payload);
    }
    setEditMode((v) => !v);
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="px-4 pb-6">
        ...
        {questions.length === 0 ? (
          <p className="text-center text-[#6b6b6b] py-12">퀴즈가 없습니다.</p>
        ) : (
