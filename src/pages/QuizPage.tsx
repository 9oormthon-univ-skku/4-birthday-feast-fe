...
const mockQuiz = Array.from({ length: 10 }).map(() => `생일 퀴즈를 작성해주세요.`);

export default function QuizPage() {
  const navigate = useNavigate();
  const [editMode, setEditMode] = useState(false);
  // 각 퀴즈마다 O/X 상태 저장 (true=O, false=X)
  const [answers, setAnswers] = useState<boolean[]>(Array(mockQuiz.length).fill(true));
            ...
            <li key={i} className="flex items-stretch gap-3">
              ...
              {editMode && (
                <div className="ml-3 flex items-center gap-2">
                  {/* O/X 토글 버튼 */}
                  <button
                    aria-label={answers[i] ? "정답" : "오답"}
                    onClick={() => toggleAnswer(i)}
                    className={`flex h-8 w-8 items-center justify-center rounded-full ${answers[i]
                      ? "bg-[#FF8B8B] text-white" // O 상태
                      : "border border-[#FF8B8B] text-[#FF8B8B]" // X 상태
                      }`}
                  >
                    {answers[i] ? "○" : "✕"}
                  </button>
...
// ---------- 아이콘 svg ---------- 
const trashBin = <svg xmlns="http://www.w3.org/2000/svg" width="22" height="24" viewBox="0 0 22 24" fill="none">
...
const editStart = <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" viewBox="0 0 23 23" fill="none">
...
const editCompl = <svg xmlns="http://www.w3.org/2000/svg" width="31" height="22" viewBox="0 0 31 22" fill="none">
  <path d="M29 2L11 20L2 11.0004" stroke="black" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" />
</svg>